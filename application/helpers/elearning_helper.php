...
function default_parser_item($add_item = array())
{
    ...
    $return = array(
        ...
        'copyright_setup'   => 'Copyright &copy; - ' . date('Y') . ' ZakariaAkhfas',
    ...
    if ($CI->uri->segment(1) != 'setup') {
        $return['copyright'] = 'Copyright &copy;' . date('Y') . ' ' . get_pengaturan('nama-sekolah', 'value') . ' by ZakariaAkhfas';
...
function belum_sepuluh_detik($datetime)
{
    $sekarang       = strtotime(date("Y-m-d H:i:s"));
    $sepuluh_d_yg_lalu = strtotime("-10 seconds", $sekarang);

    if (strtotime($datetime) > $sepuluh_d_yg_lalu) {
        return true;
    }
